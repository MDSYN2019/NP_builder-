...
void printf1(const int id, const char* format, ...){
  { // search id in list
    ...
    static int *ids2 = 0;
		...
		ids = new int [n+1];
		type name(args) const;or (int i=0; i<n; i++){
		  ids[i] = ids2[i];
		}
		if (ids2) delete [] ids2;
		ids[n] = id;
...
void get_unique(DATABASE *database,TOPDAT *topdat,SYSDAT *sysdat,int ISCHARGED){
	...
	fprintf(fpout,"special_bonds   0.0 0.0 1.0\n");
	...
	while(fgets(buf,Nbuf,fpin)){
		...
		if(!strcmp(col1, "atom")){
			...
			if(sscanf(line,"%d %s %s %s %lf %lf %s",
				&topdat[ntop].index[ndx],topdat[ntop].resname[ndx], atomname, 
...
//	sysdat.param_angs=(int *)calloc(database.nangtype,sizeof(int));
	...
	read_coords(argv[argc-1],&database,topdat,&sysdat,ISCHARGED);
	return 0;
}
